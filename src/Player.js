export default class Player {
  ...
  choice(board) {
    ...
    if (Math.random() >= 0.5 || !this.learning) {
      choice = this.bestChoice(availablePositions.map(arr => arr.join("")), this.findRotationOfBoard(board));
    }
    else {
      choice = availablePositions[Math.floor(Math.random()*availablePositions.length)].join("");
    }
